...
if(isset($_SESSION['is_adminlogin'])){
 ...
 if(isset($_REQUEST['assign'])){
  if(($_REQUEST['request_id'] == "") || ($_REQUEST['electronics_type'] == "") || ($_REQUEST['request_info'] == "") || ($_REQUEST['requestdesc'] == "") || ($_REQUEST['requestername'] == "") || ($_REQUEST['address1'] == "") || ($_REQUEST['address2'] == "") || ($_REQUEST['requestercity'] == "") || ($_REQUEST['requesterstate'] == "") || ($_REQUEST['requesterzip'] == "") || ($_REQUEST['requesteremail'] == "") || ($_REQUEST['requestermobile'] == "") || ($_REQUEST['assigntech'] == "") || ($_REQUEST['inputdate'] == "")){
   $msg = '<div class="alert alert-warning col-sm-6 ml-5 mt-2">Fill All Fileds</div>';
  } else {
   $rid = $_REQUEST['request_id'];
   $rtype = $_REQUEST['electronics_type'];
   ...
   $sql = "INSERT INTO assignwork_tb (request_id, electronics_type, request_info, request_desc, requester_name, requester_add1, requester_add2, requester_city, requester_state, requester_zip, requester_email, requester_mobile, assign_tech, assign_date) VALUES ('$rid', '$rtype', '$rinfo','$rdesc', '$rname', '$radd1', '$radd2', '$rcity', '$rstate', '$rzip', '$remail', '$rmobile', '$rassigntech', '$rdate')";
   if($conn->query($sql) == TRUE){
    ...
    <form action="" method="POST">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-8">
                <label for="electronics_type">Electronics Type</label>
                <input type="text" class="form-control" id="electronics_type" name="electronics_type" value="<?php if(isset($row['electronics_type']))echo $row['electronics_type']; ?>">
            </div>
        </div>
        <div class="form-row">
            ...
            <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" id="requestdesc" name="requestdesc" style="height:60px;" value="<?php if(isset($row['request_desc']))echo $row['request_desc']; ?>">
            </div>
        </div>
